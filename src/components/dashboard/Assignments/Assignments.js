...
function Assignments() {
  ...
  var body = localStorage.getItem("student-nation.com-tokens")
    ? parseJwt(localStorage.getItem("student-nation.com-tokens"))
    : undefined;
      ...
      {windowSize > 540 ? (
        <>
          <ul className="assignments__cards">
            {data.slice(initial, limit).map((card, ind) =>
              loading ? (
                <li>
                  <AssignmentsCardsSkeleton />
                </li>
              ) : (
                <li key={ind}>
                  <AssignmentCards
...
const cardsData = [
  ...
  {
    img: leadership,
    text: "Leadership and soft skills for startup enthusiasts",
  },
